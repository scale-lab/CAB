...
public class FairScheduler extends AbstractYarnScheduler<FSAppAttempt, FSSchedulerNode> {
    ...
    public long pastTime; //CAB
    public long currentTime; //CAB
    public int counterTime; //CAB
    public HashMap<String, JobInformation> jobsinfo; //CAB
    ...
    public FairScheduler() {
        ...
        jobsinfo = new HashMap<String, JobInformation>();
        counterTime = 0;
    ...
    //When a job is finished, we remove it from list of jobs and update the required files or variables.
    public void removeFinishedJobs() throws FileNotFoundException {

        boolean trigger = false;
        for (JobInformation jobs : jobsinfo.values()) {
            trigger = false;
            File folder = new File("/usr/local/hadoop/ApproxCapping");
            String jobID = null;

            for (File fileEntry : folder.listFiles()) {

                jobID = fileEntry.getName(); // 
                if (jobID.contains("progress")) //Progress report file. Update the related job
                {
                    continue;
                } else if (jobID.contains("killedCounter")) //killed Counter file. We do not need this kind of files in this section. Ignore it
                {
                    continue;
                } else {
                    jobID = jobID.replace(".txt", "");
                    if (jobs.jobID.equals(jobID)) { //Job is new and we do not have any record of it
                        trigger = true;
                        break;
                    }
                }
            ...
            if (trigger == false) {
                String removeJobID = jobs.jobID;
                jobsinfo.remove(jobs.jobID);
                File file2 = new File("/usr/local/hadoop/ApproxCapping/" + removeJobID + "_progress.txt");
                file2.delete();
            }
        }
    ...
    //We do not use this function in current version. We add the new jobs in the next function (updateCurrentJobs)
    
    
//    public void addNewJobs() throws FileNotFoundException {
...
//
//            jobID = fileEntry.getName(); // 
//            if (jobID.contains("progress")) //Progress report file. We do not need this kind of files in this section. Ignore it
//            {
//                continue;
//            } else if (jobID.contains("killedCounter")) //killed Counter file. We do not need this kind of files in this section. Ignore it
//            {
...
//            } else {
//                jobID = jobID.replace(".txt", "");
//                if (!jobsinfo.containsKey(jobID)) { //Job is new and we do not have any record of it
//                    JobInformation jobi = new JobInformation(); // create job
//                    Scanner scanner = new Scanner(new File(fileEntry.getAbsolutePath())); //Morteza
//                    jobi.deadline = scanner.nextInt(); //read its deadline
//                    jobi.minimumRequiredTasks = scanner.nextInt(); //read its 
//                    jobi.jobID = jobID;
//                    jobsinfo.put(jobID, jobi);
//                    scanner.close();
//                } else //Job is not new. We already have its deadline and minimum required number of tasks being executed.
//                {
//                    continue;
//                }
//            }
//
//        }
//    }//end function addNewJobs()

    
    // In this functioin we update the information of jobs such as elapsed time, executed tasks, allocated resources  and etc.
    // We also add new jobs to our list of jobs in this function.
    public void updateCurrentJobs() throws FileNotFoundException {
            ...
            if (jobID.contains("progress")) //Progress report file. Update the related job
            {
                ...
                while (sw == true) {
                    ...
                    try {
                        ...
                        jobID = jobID.replace("_progress.txt", "");
                        if (jobsinfo.containsKey(jobID)) {
                            jobsinfo.get(jobID).elapsedTime = scanner.nextInt();
                            jobsinfo.get(jobID).executedTasks = scanner.nextInt();
                            if (jobsinfo.get(jobID).executedTasks > jobsinfo.get(jobID).minimumRequiredTasks) {
                                jobsinfo.get(jobID).hasMetMinTask = true; // the job has executed minimum required tasks
                            }
                            jobsinfo.get(jobID).windowTask[jobsinfo.get(jobID).windowCounter] = jobsinfo.get(jobID).executedTasks;
                            jobsinfo.get(jobID).windowCounter++;
                            jobsinfo.get(jobID).nameofQueue = scanner.next();
                            jobsinfo.get(jobID).estimatedFinishTime = scanner.nextInt();
                            scanner.close();
                            for (FSQueue fsq : rootQueue.getChildQueues()) {
                                if (fsq.getName().equals(jobsinfo.get(jobID).nameofQueue)) {
                                    jobsinfo.get(jobID).memoryUsage = fsq.getResourceUsage().getMemory();
                                    jobsinfo.get(jobID).vcoresUsage = fsq.getResourceUsage().getVirtualCores();

                                    jobsinfo.get(jobID).MaxMemory = fsq.getMaxShare().getMemory();
                                    jobsinfo.get(jobID).MaxCore = fsq.getMaxShare().getVirtualCores();
                                    break;
                                }
                            }
                        }
    ...
    //This function is the core function of CAB Resource Allocator and is responsible for allocating resources to different jobs(queues)
    public void decisionPhase() throws IOException {
        FSQueue rootQueue = queueMgr.getRootQueue();


//        int containerMemory = (rootMetrics.getAllocatedMB() / rootMetrics.getAllocatedContainers());
//        int containerCore = (rootMetrics.getAllocatedVirtualCores() / rootMetrics.getAllocatedContainers());

        //Right now we have considered them constant. But we can use above formulas to adapt with different scenarios.
        int containerMemory = 6001;
        int containerCore = 1;

        //First calculating the required resources of each job in the form of container
        for (JobInformation job : jobsinfo.values()) {
            if ((job.elapsedTime > 0) && (job.executedTasks > 0)) // It shows that the job is running for a while and is not new
            {
                job.MaxContainer = job.MaxCore / containerCore;  //since we calculate the resources based on container, we need to know how many containers the job has in the current time
                //here, we consider a linear relationship between the number of containers and execution time of a job.
                //Hence, the below equation, estimates the required containers to finish the minimum required tasks by deadline
                //Note that the estimated finish time is the time needed to finish the "minimum required tasks"
                //estimated finish Time is calculated in CAB_Sampler_ProgressRate module and here we only use it.
                job.neededNumberofContainers = ((job.MaxContainer * job.estimatedFinishTime) / job.deadline) + 1;
                
                //if a job has already met the minimum number of task for QoR, we skip the above equation and allocate a constant share to it. 
                // Here, we allocate 2 containers, but we can change it.
                if (job.hasMetMinTask == true) {
                    job.neededNumberofContainers = 2;
                }
            }
        }// End resource requirement calculation

        
        //In the following, we first calcualte the total available containers.
        int totalClusterContainers = clusterResource.getVirtualCores() / containerCore;
        
        //Then, we calculate the total required containers of jobs with respect to previous calculations.
        int totalRequiredContainers = 0;
        for (JobInformation job : jobsinfo.values()) {
            totalRequiredContainers += job.neededNumberofContainers;
        }

        
        //If There is enough resources to satisfy all the jobs, we will allocate the required resources of each job, and then if there are still some resources available,
        //We distribute them evenly among the jobs.
        //In might be a little bit different from problem formulation, where we always allocate the resources proportionally.
        //In the future versions, we can modify this part and make sure even in this case the resources are allocated proportionally.
        if (totalRequiredContainers <= totalClusterContainers && totalRequiredContainers > 0) 
        {
            for (JobInformation job : jobsinfo.values()) {
                job.MaxContainer = job.neededNumberofContainers;
            }

            int remainingContainers = totalClusterContainers - totalRequiredContainers;
        ...
        //In case that total required containers is more than total avaialbe containers (totalClusterContainers)
        //we have to allocate the resources proportionally to jobs based on their required containers.
        //Here we again exclude the jobs that have met minimum task requirement and give them a constant number of containers
        //and do not consider them in the remaining calculations. We give them 2 containers. But again this number can change.
        // please not that when we calculate the resource share of each job, we obtain a float number. However, we know that always the number of containers 
        //that can be allocated to a job is integer. Hence, after calculating the shares and rounding them up. We check to see if the total assigned containers is 
        //equal to the total available containers or not. In case it is more than that, we need to reduce the number of assigned containers to each job and we do it 
        //by decreasing the number of conainters of each job by one unit, until the allocated resources and available ones are equal.
        
        else if (totalRequiredContainers > totalClusterContainers && totalRequiredContainers > 0)//total Required containers is more than availabe ones.
        {
            //first assign two containers to each of the jobs that have already met the minimum required tasks;
            int countJobMetMin = 0;
            for (JobInformation job : jobsinfo.values()) {
                if (job.hasMetMinTask == true) {
                    countJobMetMin++;
                    job.MaxContainer = 2;
                    totalClusterContainers -= 2;
                    totalRequiredContainers -= 2;
                }
            }

            if (countJobMetMin == jobsinfo.size()) //all the jobs has met minimum
            {
                while (totalClusterContainers != 0) {
                    for (JobInformation job : jobsinfo.values()) {

                        job.MaxContainer++;
                        totalClusterContainers--;
                        if (totalClusterContainers == 0) {
                            break;
                        }
                    }
                }
                ...
                int totalShare = 0;
                for (JobInformation job : jobsinfo.values()) {
                    if (job.hasMetMinTask == false) {
                        jobshare = ((float) job.neededNumberofContainers / totalRequiredContainers) * totalClusterContainers;
                        job.MaxContainer = (int) jobshare;
                        if (job.MaxContainer == 0) {
                            job.MaxContainer = 1;
                        }
                        totalShare += job.MaxContainer;
                    }
                }

                if ((totalShare > totalClusterContainers) && totalShare > 0) {
                    int extracontainers = totalShare - totalClusterContainers;
                    while (extracontainers != 0) {
                        for (JobInformation job : jobsinfo.values()) {
                            if (job.hasMetMinTask == false && job.MaxContainer > 1) {
                                job.MaxContainer--;
                                extracontainers--;
                            }
                ...
                } else if ((totalShare < totalClusterContainers) && totalShare > 0) {
                    int extracontainers = totalClusterContainers - totalShare;
        ...
        } //end total required container is more than available ones
        
//        for (JobInformation job : jobsinfo.values()) {
//            rootQueue.scheduler.getAllocationConfiguration().setMinShare(job.nameofQueue, containerMemory, containerCore);
//            rootQueue.scheduler.getAllocationConfiguration().setMaxShare(job.nameofQueue, (job.MaxContainer * containerMemory) - 100, job.MaxContainer * containerCore);
//        }
        
        // New August 27th , 2017
        String queueus[] = new String[100];
        int resourceQuota[] = new int[100];
        int tempCounter = 0;
        for (JobInformation job : jobsinfo.values()) {
            int triger = 0;
            for (int  i = 0; i < tempCounter; i++)
            {
                if (job.nameofQueue.equals(queueus[i]))
                {
                    resourceQuota[i] += job.MaxContainer;
                    triger = 1;
                }
            }
            if (triger == 0)
            {
                queueus[tempCounter] = job.nameofQueue;
                resourceQuota[tempCounter] = job.MaxContainer;
                tempCounter++;
            }
        ...
        //calculating extras resources to give to queues
        int tempResource = 0;
        for (int k = 0 ; k < tempCounter; k++)
        {
            tempResource += resourceQuota[k];
        }
        if (tempResource < totalClusterContainers)
        {
            int tempIndex = 0;
            for (int m = 0 ; m < totalClusterContainers - tempResource; m++) 
            {
                resourceQuota[tempIndex]++;
                tempIndex++;
                if (tempIndex == tempCounter)
                    tempIndex = 0;
            }
        }
        ...
        {
            rootQueue.scheduler.getAllocationConfiguration().setMinShare(queueus[j], containerMemory, containerCore);
            rootQueue.scheduler.getAllocationConfiguration().setMaxShare(queueus[j], (resourceQuota[j] * containerMemory) - 100,resourceQuota[j] * containerCore);
        }
        
        //End New August 27th, 2017
        
        
    }

    // we can log different features of jobs and queues in this function. A timestamp is used to syncronize the logs between different jobs.
    //Now, we log the resource share of job (job.MaxContainer), elapsed time of job, number of executed jobs within elapsed time (jb.executedTasks), 
    //minimum required task of job and deadline of job.
    public void loggingResourceUsage() {

        Date date2 = new Date();
        long timeNow = date2.getTime();
        timeNow = timeNow / 1000;

        for (JobInformation job : jobsinfo.values()) {
            try (FileWriter fw = new FileWriter("/usr/local/hadoop/JobsLog/" + job.jobID + ".txt", true);
                    BufferedWriter bw = new BufferedWriter(fw);
                    PrintWriter out = new PrintWriter(bw)) {
                out.println(timeNow + " " + job.nameofQueue + " " + job.MaxContainer + " "
                        + job.elapsedTime + " " + job.executedTasks + " " + job.minimumRequiredTasks + " " + job.deadline);
                out.close();
                bw.close();
                fw.close();
                //more code
    ...
    //Right now, we do not use this function. But, in earlier versions we needed it to make sure that the 
    public void safetyCheck() throws IOException { // balance the resources of queues when something on normal happens
        FSQueue rootQueue = queueMgr.getRootQueue();
        int totalMem = clusterResource.getMemory();
        int totalVcore = clusterResource.getVirtualCores();

        File file2 = new File("/usr/local/hadoop/ClusterResource.txt");
        ...
        bw2.write("Cluster Memory = " + clusterResource.getMemory() + "\n");
        bw2.write("Cluster CPU = " + clusterResource.getVirtualCores() + "\n");
        bw2.write("Root Queue Size = " + rootQueue.getChildQueues().size() + "\n");

        for (FSQueue fsq : rootQueue.getChildQueues()) {
            bw2.write("Fsq = " + fsq.getName() + "\n");
        }
        ...
        int qmem = 0;
        int qcore = 0;
        int numberofQueues = rootQueue.getChildQueues().size() - 1;
        for (FSQueue fsq : rootQueue.getChildQueues()) {
            if (!fsq.getName().equals("root.default")) {
                qmem += fsq.getMaxShare().getMemory();
                qcore += fsq.getMaxShare().getVirtualCores();
            }
        }
        ...
        if (totalVcore != qcore) {
            for (FSQueue fsq : rootQueue.getChildQueues()) {
                if (!fsq.getName().equals("root.default")) {
                    fsq.scheduler.getAllocationConfiguration().setMaxShare(fsq.getName(), (totalMem / numberofQueues) - 100,
                            totalVcore / numberofQueues);
                }

            }

            int remainingCore = totalVcore % numberofQueues;

            for (FSQueue fsq2 : rootQueue.getChildQueues()) {
                if (!fsq2.getName().equals("root.default")) {
                    fsq2.scheduler.getAllocationConfiguration().setMaxShare(fsq2.getName(), (totalMem / numberofQueues) - 100,
                            fsq2.getMaxShare().getVirtualCores() + 1);
                    remainingCore--;
                }
                if (remainingCore == 0) {
                    break;
    ...
    protected synchronized void update() throws IOException {
        ...
        if (((currentTime - pastTime) / 1000) >= 20) { // the number (here 20) determines the decision making intervals in seconds
            ...
            decisionPhase();
            //safetyCheck();
            loggingResourceUsage();
            loggingResourceUsage(); // just to have more logs (it can be eliminated)
            loggingResourceUsage(); // just to have more logs (it can be eliminated)
            loggingResourceUsage(); // just to have more logs (it can be eliminated)
